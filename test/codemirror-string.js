exports.codemirror30 = "// All functions that need access to the editor's state live inside\n\
...
\n\
  function onFocus(cm) {\n\
    ...
    if (!cm.view.focused) {\n\
      ...
      if (cm.display.scroller.className.search(\"\") == -1)\n\
...
\n\
    if (cm.options.autoClearEmptyLines && posEq(sel.from, sel.to)) {\n\
      ...
      if (head.line != sel.from.line && sel.from.line < doc.size) {\n\
        ...
        if (\"\".test(oldLine.text))\n\
          setTimeout(operation(cm, function() {\n\
            //if (oldLine.parent && /^\\s+$/.test(oldLine.text)) {\n\
            if (oldLine.parent && \"\".test(oldLine.text)) {\n\
...
\n\
    ...
    curSpaceString = line.text.match(\"\")[0], indentation;\n\
    if (how == \"smart\") {\n\
  ...
  function findWordAt(line, pos) {\n\
    ...
    if (line) {\n\
      ...
      var check = isWordChar(startChar) ? isWordChar :\n\
        \"\".test(startChar) ? function(ch) {return \"\".test(ch);} :\n\
        ///\\s/.test(startChar) ? function(ch) {return /\\s/.test(ch);} :\n\
      //function(ch) {return !/\\s/.test(ch) && !isWordChar(ch);};\n\
      function(ch) {return !\"\".test(ch) && !isWordChar(ch);};\n\
...
\n\
  CodeMirror.resolveMode = function(spec) {\n\
    ...
    else if (typeof spec == \"string\" && \"\".test(spec))\n\
  ...
  var commands = CodeMirror.commands = {\n\
    ...
    goLineStartSmart: function(cm) {\n\
      ...
      if (!order || order[0].level == 0) {\n\
        ...
        var firstNonWS = Math.max(0, line.text.search(\"\"));\n\
...
\n\
  StringStream.prototype = {\n\
    ...
    eatSpace: function() {\n\
      ...
      while (\"\".test(this.string.charAt(this.pos))) ++this.pos;\n\
      return this.pos > start;\n\
    },\n\
...
\n\
  function isWordChar(ch) {\n\
    ...
    return \"\".test(ch) || ch.toUpperCase() != ch.toLowerCase();\n\
  }\n\
\n\
...
\n\
  ...
  var splitLines = \"\\n\\nb\".split(\"\").length != 3 ? function(string) {\n\
...
\n\
    ...
    var isNeutral = \"\", isStrong = \"\", countsAsLeft = \"\", countsAsNum = \"\";\n\
\n\
...
\n\
      ...
      if (order[0].level == 1 && (m = str.match(\"\"))) {\n\
        order[0].from = m[0].length;\n\
        order.unshift({from: 0, to: m[0].length, level: 0});\n\
      }\n\
      //if (lst(order).level == 1 && (m = str.match(/\\s+$/))) {\n\
      if (lst(order).level == 1 && (m = str.match(\"\"))) {\n\
